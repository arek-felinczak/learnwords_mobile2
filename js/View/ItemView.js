ItemView = Backbone.View.extend({
    render:function(nr, navHtml, cat, collection) {
        var item = this.model; 
        var index = -1;
        _.each(collection.models, function(o, i) { 
            if (o.attributes.Id === item.attributes.Id) {
                index = i;
            }            
        });
        ...
        var pageList = new Vocabulary.Pager(collection, 1);
        var pager = pageList.render('#item/{categoryId}/{page}/' + nr + 
            (cat.attributes.Id === 0 ? "/fav" : ""), 
            index + 1, true);
        var vm = {
            src: url,
            showPager: true,
